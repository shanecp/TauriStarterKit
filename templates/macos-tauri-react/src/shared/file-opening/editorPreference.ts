...
export function getStoredEditorAppName(): string {
...
export function saveEditorAppName(value: string): string {
  const normalized = normalizeEditorAppName(value);
  return writeSetting(EDITOR_APP_SETTING_KEY, normalized);
}

export function resetEditorAppName(): string {
  removeSetting(EDITOR_APP_SETTING_KEY);
